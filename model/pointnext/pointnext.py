"""
Adapted from PointNeXt implementation in repo: https://github.com/guochengqian/openpoints
"""
...
# from model.pointnext.layers import CHANNEL_MAP, create_act, create_convblock1d, create_convblock2d, \
#                                    create_grouper, furthest_point_sample, get_aggregation_feautres, \
...
class LocalAggregation(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(channels) - 1):   # number of layers in each block
            convs.append(create_convblock2d(
                channels[i], channels[i + 1], norm_args=norm_args,
                act_args=None if i == (len(channels) - 2) and not last_act else act_args, **conv_args
            ))
...
class PointNext(nn.Module):
    def __init__(
        ...
        conv_args=None,
        aggr_args={'feature_type': 'dp_fj', 'reduction': 'max'},
        group_args={'NAME': 'ballquery', 'radius': 0.1, 'nsample': 32},
        norm_args={'norm': 'bn'},
        act_args={'act': 'relu'},
        sampler='fps',
        expansion=4,
        ...
        logger.debug(f"PointNext: radius = {self.radii}, nsample = {self.nsample}")
        ...
        for i in range(len(self.num_blocks)):
            ...
            encoder.append(self._make_enc(
                block, channels[i], self.num_blocks[i], stride=self.strides[i], group_args=group_args, 
        ...
        if isinstance(param, list):
            ...
            for i, stride in enumerate(self.strides):
                if stride == 1:
                    ...
                    param_list.append(
                        [param] + [param * param_scaling] * (self.num_blocks[i] - 1)
                    )
                    param *= param_scaling
        return param_list
    
    def _make_enc(self, block, channels, num_blocks, stride, group_args, is_head=False):
        layers = []
    ...
    def forward_cls_feat(self, x):
        ...
        if x.ndim == 4:
            ...
            raise ValueError("Point cloud input shape incorrect, ndim should be either 3 or 4.")
