...
from data.eai import CLIPORT_ACTION_SPACE_DETOKENIZE, _CLIPORT_ACTION_SPACE_U, _CLIPORT_ACTION_SPACE_V, \
...
#             v: k for k, v in zip(list(HABITAT_ACTION_SPACE.values()), list(VICUNA_ACTION_TOKENS.keys())[:len(HABITAT_ACTION_SPACE)])
#         }
       
#         for i in range(batch_size):
#             action = HABITAT_ACTION_SPACE_DETOKENIZE[data_dict['output_gt'][i]]
#             pred_action = HABITAT_ACTION_SPACE_DETOKENIZE[ data_dict['output_txt'][i]]
...
class CLIPortEvaluator(ObjNavEvaluator):
    ...
    def reset(self):
        self.eval_dict = {
            ...
            'action_error_pose0': [], 'action_error_pose1': [],
        }
        self.total_count = 0
        self.save_results = []
    ...
    def parse_action_cliport(text, obs=None):
        vocab = list(_CLIPORT_ACTION_SPACE_U.values()) + list(_CLIPORT_ACTION_SPACE_V.values()) + list(_CLIPORT_ACTION_SPACE_ZROT.values())
    ...
    def tokenize(text, prefix, vocab):
        ...
        while i < len(after_prefix):
            ...
            for token in vocab:
                ...
                if after_prefix[i:i+token_length] == token and token_length > max_length:
                    match = token
