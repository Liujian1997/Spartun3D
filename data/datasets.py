...
class LeoBase(Dataset):
    ...
    def load_scannet(self, scan_id):
        scan = {}
        pcd_data = torch.load(os.path.join(self.scannet_base, 'pcd_with_global_alignment', f'{scan_id}.pth'))
...
class LeoMP3DObjNav(LeoBase):
    ...
    def __init__(self, cfg, split):
        ...
        self.scan_data = {}
        self.train_scan = ['17DRP5sb8fy']

    def load_demos(self, anno_dir):
        all_data = []
        num_demos = 0
        #files = glob.glob(os.path.join(anno_dir, 'demos/*/*/demo.json'))
        anno_dir = "/egr/research-hlr2/zhan1624/leo_data/nav_data/"
        files = glob.glob(os.path.join(anno_dir, 'train/*/*/demo.json'))
        #files = glob.glob(os.path.join(anno_dir, 'demos/*/*/demo.json'))

        flag = 0
        count_dict = {}
        for file in files:
            scene_id = file.split("/")[-3]
            if self.split == 'train':
                if scene_id in self.heldout_scenes:
                    continue
            else:
                if scene_id not in self.heldout_scenes:
                    continue

            # if scene_id not in count_dict:
            #     count_dict[scene_id] = 1
            # else:
            #     if count_dict[scene_id] < 100:
            #         count_dict[scene_id] += 1
            #     else:
            #         continue
            ...
            # if self.split == 'train':
            #     if traj['scene_id'] not in self.train_scan:
            #         continue
            # else:
...
class LeoCLIPort(LeoBase):
    ...
    def load_demos(self, anno_dir):
        anno_dir = "/egr/research-hlr2/zhan1624/leo_data/annotations/instruction/eai_data_example/cliport/leo-manip/"
        ...
        if os.path.exists(index_file):
            ...
            anno_dir = "/egr/research-hlr2/zhan1624/leo_data/annotations/instruction/eai_data_example/cliport/leo-manip/separating-piles-seen-colors-train/"
            files = glob.glob(os.path.join(anno_dir, '*.pkl'))
            for file in files:
                if self.split == 'train':
    ...
    def __getitem__(self, index):
        ...
        file = "/egr/research-hlr2/zhan1624/leo_data/annotations/instruction/eai_data_example/cliport/leo-manip/separating-piles-seen-colors-train/"+file.split('/')[-1]
...
@DATASET_REGISTRY.register()
class Spartun3RScanSitCap(LeoBase):
    situation_pool = Leo_situation_pool
    instruction_pool = Leo_scenecap_instruction_pool
    def __init__(self, cfg, split):
        super().__init__()
        self.split = 'train' if split == 'train' else 'val'
        self.rscan_base = cfg.data.sitrscan_cap.rscan_base
        self.num_points = cfg.data.sitrscan_cap.num_points
        self.max_obj_len = cfg.data.sitrscan_cap.max_obj_len
        self.max_caption_length = int(cfg.llm.max_out_len / LLAMA_TOKEN_SENT_RATIO)

        logger.info(f"Loading Situated Caption {split}-set language")
        self.scan_ids, self.lang_data, self.scan_insts = self.load_anno(cfg.data.scene_cap.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading Sitated Caption{split}-set language, collected {len(self.scan_ids)} data")
    ...
    def load_anno(self, anno_dir):
        ...
        for item_key, item_value in anno_data.items():
            #scan_ids.append(item_key.split("_")[0])
            connect_key = item_value['query']
            insts = [s for s in connect_key.values()]
            merged_insts = [item for sublist in insts for item in sublist]
            merged_insts = [int(m.split("_")[1]) for m in merged_insts]
            merged_insts.insert(0, int(item_key.split("_")[1]))
            if int(item_value["refer"]) not in merged_insts:
                merged_insts.insert(0, int(item_value["refer"]))
            if item_value['relation']:
                for each_rel in item_value['relation']:
                    each_rel = int(each_rel.split("_")[1])
                    if each_rel not in merged_insts:
                        merged_insts.append(each_rel)
            assert len(merged_insts) == len(set(merged_insts))
            if len(merged_insts)>self.max_obj_len:
                merged_insts = merged_insts[:self.max_obj_len]
            merged_insts = list(set(merged_insts))
            cap = item_value['caption']
            pos = item_value['pos']
            rot = item_value['rot']
       
            scan_ids.append(item_key.split("_")[0])  
            scan_caps.append({
                's': item_value['situation'],   # list of str
                'c': cap,   # str
                'pos': np.array(pos),
                'rot':np.array(rot)
                #'a': item_value['answer'],   # list of str
                #'pos': np.array(list(item['position'].values())),   # array (3,)
                #'rot': np.array(list(item['rotation'].values())),   # array (4,)
            })
            all_insts.append(merged_insts)
        return scan_ids, scan_caps, all_insts
    ...
    def __getitem__(self, index):
        scan_id = self.scan_ids[index]
        sqa_dict = self.lang_data[index]
        #situation   = self.scan_situation[index]
        situation = sqa_dict['s']   # list of str
        caption = sqa_dict['c']   # list of str
        pos = sqa_dict['pos']
        rot = sqa_dict['rot']
        scan_insts = self.scan_insts[index]
        ...
        obj_fts, obj_locs, (pos, rot) = self.preprocess_pcd(selected_obj_pcds, return_anchor=False, situation=(pos, rot))
        ...
        data_dict = self.get_prompts(
            instruction=random.choice(self.instruction_pool).strip('.').strip("?")+" from your current position.",
            situation=random.choice(self.situation_pool) + ' ' + self.convert_person_view(situation),
        )
        data_dict.update({
            'source': '3rscan',
...
@DATASET_REGISTRY.register()
class Spartun3RScanSitQA(LeoBase):
    situation_pool = Leo_situation_pool
    ...
    def load_anno(self, anno_dir):
        ...
        for item_key, item_value in anno_data.items():
            ...
            merged_insts = [item for sublist in insts for item in sublist]
            merged_insts = [int(m.split("_")[1]) for m in merged_insts]
            merged_insts.insert(0, int(item_objid))
            if int(item_value["refer"]) not in merged_insts:
                merged_insts.insert(0, int(item_value["refer"]))
            pos = item_value['pos']
            rot = item_value['rot']
            ...
            for qa_id, qa in enumerate(item_value['qa']):
                ...
                sqa_annos.append({
                    's': item_value['situation'].lower(),   # list of str
                    'q': qa[0].lower(),   # str
                    'a': qa[2].lower(),   # list of str
                    'pos': np.array(pos),
                    'rot':np.array(rot)
                })
                sqa_insts.append(merged_insts)
            count += 1

        return scan_ids, sqa_annos, sqa_insts
    ...
    def __getitem__(self, index):
        ...
        question = sqa_dict['q']   # str
        answer_list = sqa_dict['a']   # list of str
        scan_insts = self.sqa_insts[index]
        pos = sqa_dict['pos']   # array, (3,)
        rot = sqa_dict['rot']   # array, (4,)

        # load pcds
        if scan_id not in self.scan_data:
            self.scan_data[scan_id] = self.load_rscan(scan_id.split("_")[0])
        ...
        data_dict.update({
            ...
            'img_fts': torch.zeros(3, 224, 224),
            'img_masks': torch.LongTensor([0]).bool(),
            'output_gt': answer_list.strip(".") if self.split == 'train' else [answer_list.strip(".")],
            'sqa_type': question_type,
        })
...
@DATASET_REGISTRY.register()
class Spartun3RScanAfford(LeoBase):
    situation_pool = Leo_situation_pool
    ...
    def __init__(self, cfg, split):
        ...
        logger.info(f"Loading Spartun3DAfford {split}-set language")
        self.scan_ids, self.lang_data, self.sqa_insts = self.load_anno(cfg.data.sitrscan_afford.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading Spartun3DAfford {split}-set language, collected {len(self.scan_ids)} data")
    ...
    def load_anno(self, anno_dir):
        ...
        anno_file = os.path.join(anno_dir, "afford_%s.json"%self.split)
        ...
        for item_key, item_value in anno_data.items():
            ...
            for qa in item_value['qa']:
                if not qa[2].endswith("."):
                     continue
                scan_ids.append(item_scan)
                sqa_annos.append({
                    's': item_value['situation'].lower(),   # list of str
                    'q': qa[0].lower(),   # str
                    'a': qa[2].lower(),   # list of str
...
@DATASET_REGISTRY.register()
class Spartun3RScanPlanning(LeoBase):
    situation_pool = Leo_situation_pool
    ...
    def __init__(self, cfg, split):
        ...
        logger.info(f"Loading Spartun3DPlanning {split}-set language")
        self.scan_ids, self.lang_data, self.sqa_insts = self.load_anno(cfg.data.sitrscan_planning.anno_dir)
        # scan_ids may be repeatitive
        logger.info(f"Finish loading Spartun3DPlanning {split}-set language, collected {len(self.scan_ids)} data")
    ...
    def load_anno(self, anno_dir):
        ...
        anno_file = os.path.join(anno_dir, "plan_%s.json"%self.split)
...
class LeoMix(Dataset):
    mapping = {
        ...
        'sitrscan_cap':Spartun3RScanSitCap,
        'sitrscan_qa':Spartun3RScanSitQA,
        'sitrscan_afford':Spartun3RScanAfford,
        'sitrscan_planning':Spartun3RScanPlanning,
    }
