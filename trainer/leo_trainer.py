import os
import sys
sys.path.append("/home/yue/projects/embodied-generalist/")
...
class LeoTrainer():
    def __init__(self, cfg):
            ...
            if cfg.task[task_name] and 'dataset' in cfg.task[task_name]:
                for mode in cfg.task[task_name].mode:
                    self.data_loaders[mode][task_name] = build_dataloader_leo(
                        cfg=cfg, split=mode, dataset_name=cfg.task[task_name].dataset,
                        dataset_wrapper_name=cfg.task[task_name].dataset_wrapper, dataset_wrapper_args=cfg.task[task_name].dataset_wrapper_args,
                        dataloader_args=cfg.task[task_name].train_dataloader_args if mode == 'train' else cfg.task[task_name].eval_dataloader_args,
                    )
                if 'evaluator' in cfg.task[task_name]:
                    self.evaluators[task_name] = build_eval_leo(cfg, task_name, cfg.task[task_name].evaluator)
                    self.eval_metrics[task_name] = 0

        assert len(self.data_loaders['train']) <= 1, "LEO requires only one training set"
        ...
        total_steps = ceil(len(list(self.data_loaders['train'].values())[0]) / gradient_accumulation_steps) * cfg.task.training.epochs
        ...
        if self.mode == 'train':
            ...
            for epoch in range(start_epoch, self.epochs):
                ...
                if (epoch + 1) % self.val_interval == 0:
                    ...
                    if is_best:
                        print("#######################"+str(epoch)+"#######################")
                        self.save('best_%s.pth'%str(epoch), model_only=True)   # save the best checkpoint
                        self.accelerator.wait_for_everyone()
                        
                if epoch % 1 == 0:
                    self.model.eval()
                    self.save(str(epoch), model_only=True)   # save the best checkpoint
                    self.accelerator.wait_for_everyone()
