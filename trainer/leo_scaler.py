...
class LeoScaler(LeoTrainer):
    def __init__(self, cfg):
        ...
        train_set = get_dataset_leo(
            cfg=cfg, split='train', dataset_name=cfg.task.leomix.dataset,
        ...
        for train_subset in train_subsets:
            self.data_loaders['train'].append(
                DataLoader(
                    train_subset, batch_size = cfg.dataloader.train.batchsize, num_workers = cfg.dataloader.train.num_workers,
                    collate_fn = getattr(train_subset, 'collate_fn', default_collate), pin_memory = True,
                    shuffle = True, drop_last = True,
                )
            )

        self.data_loaders['val'] = build_dataloader_leo(
            cfg=cfg, split='test', dataset_name=cfg.task.leomix.dataset,
            dataset_wrapper_name=cfg.task.leomix.dataset_wrapper, dataset_wrapper_args=cfg.task.leomix.dataset_wrapper_args,
            dataloader_args=cfg.task.leomix.eval_dataloader_args,
        )

        # prepare dataloaders
        self.data_loaders['train'] = [self.accelerator.prepare(sub_loader) for sub_loader in self.data_loaders['train']]
        self.data_loaders['val'] = self.accelerator.prepare(self.data_loaders['val'])
        ...
        total_steps = sum([ceil(len(sub_loader) / gradient_accumulation_steps) for sub_loader in self.data_loaders['train']])
    ...
    def train_step(self, epoch):
        ...
        self.model.train()
        loader = self.data_loaders['train'][epoch]   # the only difference to LeoTrainer.train_step()
    ...
    def val_step(self, epoch):
        ...
        for data_dict in loader:
            ...
            if not isinstance(data_dict['output_gt'][0], str):
                data_dict['output_gt'] = [random.choice(answer_list) for answer_list in data_dict['output_gt']]
        ...
        logger.info(f"Finish validation epoch {epoch+1}, test set loss: {loss_dict}")
