...
class LeoProber(LeoTrainer, LeoBase):
    def __init__(self, cfg):
        ...
        # prepare data
        self.sources = [cfg.probe.sources] if isinstance(cfg.probe.sources, str) else list(cfg.probe.sources)
        self.scene_ids = [cfg.probe.scene_ids] if isinstance(cfg.probe.scene_ids, str) else list(cfg.probe.scene_ids)
        self.situations = [cfg.probe.situations] if isinstance(cfg.probe.situations, str) else list(cfg.probe.situations)
        self.instructions = [cfg.probe.instructions] if isinstance(cfg.probe.instructions, str) else list(cfg.probe.instructions)

        self.num_samples = max(len(self.sources), len(self.scene_ids), len(self.situations), len(self.instructions))
        ...
        assert len(self.sources) == len(self.scene_ids) == len(self.situations) == len(self.instructions)

        self.data_dict = {
            'source': self.sources,
            'scene_id': self.scene_ids,
            'prompt_before_obj': [self.role_prompt + self.situation_prompt.format(situation=s) for s in self.situations],
            ...
            'obj_masks': [],
            'obj_locs': [],
            'anchor_locs': torch.zeros(self.num_samples, 3, device=device),
            'img_fts': torch.zeros(self.num_samples, 3, 224, 224, device=device),
            'img_masks': torch.zeros(self.num_samples, 1, dtype=torch.bool, device=device),
        }
        for instruction in self.instructions:
            if 'USER:' in instruction:
                ...
                # single question
                self.data_dict['prompt_after_obj'].append(self.task_prompt.format(instruction=instruction))
        ...
        self.data_dict['obj_fts'] = torch.stack(self.data_dict['obj_fts']).to(device)
        self.data_dict['obj_masks'] = torch.stack(self.data_dict['obj_masks']).to(device)
        self.data_dict['obj_locs'] = torch.stack(self.data_dict['obj_locs']).to(device)
    ...
    def load_scene(self, source, scene_id):
        ...
        obj_fts = pad_tensors(obj_fts, lens=self.max_obj_len, pad=1.0).float()   # O, num_points, 6
        obj_masks = (torch.arange(self.max_obj_len) < len(obj_locs))   # O
        obj_locs = pad_tensors(obj_locs, lens=self.max_obj_len, pad=0.0).float()   # O, 6
    ...
    def run(self):
        for i in trange( ceil( self.num_samples / self.batch_size ) ):
            ...
            for k in self.data_dict.keys():
                batch_data_dict[k] = self.data_dict[k][self.batch_size*i: self.batch_size*(i+1)]
            ...
            for j in range(self.batch_size):
                ...
                if self.save_obj_tokens:
                    torch.save({
                        'obj_tokens': output['obj_tokens'][j].unsqueeze(0).cpu(),
                        'obj_masks': output['obj_masks'][j].unsqueeze(0).cpu(),
                    }, os.path.join(self.save_dir, f'{self.sources[idx]}-{self.scene_ids[idx]}.pth'))
